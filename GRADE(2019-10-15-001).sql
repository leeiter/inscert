...
-- 간혹 표준 SQL의 PK 지정 방식이 안되는 DBMS 가 있다
-- 이때는 별도의 CONSTRAINT 추가 방식으로 지정해야 한다.
-- 이 방식은 다수의 칼럼으로 PK를 지정할 떄도 사용한다.
...
학생이름-국어-영서-수학-과학... 형식으로 펼쳐서 보기
*/
-- 1. tbl_score에 저장된 과목명들을 확인
-- 데이터에 어떤 과목들이 저장되어있나 확인
...
/*
    성적테이블을
    각 과목이름으로 칼럼은 만들어서 생성을 하면
    데이터를 추가하거나, 단순 조회를 할때는 상당히 편리하게 사용할 수 있다.
...
PIVOT (
    ...
    IN ( -- 펼쳤을때 보여질 칼럼 이름 목록
        '과학' AS 과학, '국사' AS 국사, '국어' AS 국어,
        '미술' AS 미술, '수학' AS 수학, '영어' AS 영어)
)
ORDER BY s_std;
