...
2. 엑셀 데이터를 단순히  DBMS의 테이블로 구현
    ...
    - 취미가 3개 미만인 학생은 사용하지 않는 칼럼이 있어 낭비가 되는 상황
=====================================================
...
-- 아래의 두 SQL 문 결과 값이 같으면 정상적으로 수행 완료
SELECT
    COUNT(*)
FROM tbl_score;
SELECT
...
-- tbl_subject의 sbcode 칼럼과 구조(형식)이 같은 칼럼을 추가
...
-- UPDATE 수행 후 검증
SELECT
    sc.s_sbcode, sb.sb_code, sc.s_subject, sb.sb_name
FROM tbl_score SC, tbl_subject SB
...
-- TABLE을 JOIN 할떄 TABLE들에 칼럼 이름이 같은 이름이 존재하면
-- 반드시 칼럼이 어떤 TABLE에 있는 칼럼인지 명시를 해 주어야
-- 문법적 오류가 발생하지 않는다.
-- 그래서 TABLE을 설계할 때 가급적 접두사를 붙여서 만드는 것이 좋고
-- 그렇더라도 JOIN을 할 떄 TABLE alias를 설정하여
...
FROM Table1, Table2
    WHERE dept = num
라는 형식으로 SQL 을 작성하면 num이 누구의 Table인지 알수 없어서
문법적인 오류가 발생한다.

SELECT T1.num AS , T1.name, T1.addr, T1.dept
        T2.num AS , T2.subject, T2.pro
