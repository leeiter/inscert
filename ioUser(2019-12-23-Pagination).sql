...
/*
pagination
전체 레코드를 모두 읽어서 화면에 ㅂ ㅗ여주는 것은 실제상황에서는
매우 비 효율적이다
메모리 문제가 될 수 있고,
...
SELECT
    *
FROM tbl_product ORDER BY p_code;
...
SELECT
    *
FROM tbl_product ORDER BY p_code DESC;
...
from 절에 사용한 SELECT 를 inline view 라고 한다.

FIRST_ROWS
WHERE 절에서 ROWNUM의 가상 칼럼 값을 N 이하로 설정하면
처음부터 NRKwl 데이터를 가져오는데 최소 비용을 투입하는
알고리즘을 작동하여라 하는 의미
*/
...
-- 최적화 알고고리즘을 작동시키도록 구조가 만들어져 있어서
-- 시작 값을 WHERE로 제한하면
...
옵티마이져를 하지 않은 것보다 늦게 나오는 경구가 있다.
