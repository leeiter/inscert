...
-- 현재 이 화면에서는 명령을 수행하는 사용자는 SYSDBA 권한을 가진 사람이다.
...
-- 정상적으로 수행되었나는 확인 용도로 사용된다.

-- 오라클에서는 관리자가 새로운 사용자를 생성하면
-- 아무런 권한도 없는 상태로 둔다.
-- 새로 생성된 사용자는 id, password를 입력해도 접속 자제가 되지 않는다.
...
-- 일단, 자세한 권한 정책을 학습하기 앞서 표준SQL 등을 학습하기 용이 하도록
