...
-- 관리자 계정으로 접속된 상태에서 TableSpace, User 등을 생성한다
-- DDL(Data definition Lang.)

-- 디엘토리 구분문자는 \나 /를 사용 가능하다.
-- 윈도우에서는 \를 사용하지만 다른 보편적인 운영체제에서는 /를 사용한다
-- 호환성을 위해서 \를 /로 변경하여 위치를 표시한다.
-- 맨앞에 /만 사용하면 윈도우에서는 C:/와 같은 의미로 사용된다.
...
-- user2 table을 생성하고 데이터를 저장할 때 user2_DB tablespace를 사용하도록 지정

-- 만약 DEFAUL TABLESPACE를 지정하지 않으면
-- user2 사용자가 table을 생성하고 데이터를 저장할 때
-- 그데이터들을 오라클DBMS의 SYSTEM 영역에 저장을 해 버린다.
-- 작은 규모의 프로젝트에서는 큰제는 없으나
-- 실무에서는 매우 좋지 않은 방법이다.
