...
-- 검증을 하기 위해 tbl_iolist와 tbl_product 를 EQ JOIN해서 조회
SELECT
    io_inout, COUNT(*)
...
-- 상품들의 매입단가를 확인
SELECT
    io_inout, IO.io_pname, io.io_price, COUNT(*)
FROM tbl_iolist IO, tbl_product P
WHERE IO.io_pname = p.p_name
    AND IO.io_inout = 1
...
-- NULL인 값이 있다.
-- 어떤상품은 매입만, 어떤 상품은 매출만 된 경우

/*
    매입단가에서 매출단가 생성하기
    공산품일경우 매입단가의 약 18% 를 더해서 매출단가를 계산
    그리고 여기에 10%의 VAT를 붙여서 다시
...
UPDATE tbl_product
SET p_iprice = ROUND((p_oprice / 1.1) * 0.82, 0)
WHERE p_iprice IS NULL;

UPDATE tbl_product
SET p_oprice = ROUND((p_iprice + (p_iprice * 0.18)) * 1.1, 0)
