...
-- 테이블 명명 패턴 : 일반적으로 테이블을 만들 때 테이블 이름 앞에 접두사 tbl_ 사용한다.
...
-- 이후에 tbl_student 테이들베엇 0100의 학생데이터를 조회 했더니
-- 데이터가 2개가 조회되었다.
...
-- 학생 데이터를 추가하는 과정에서
-- 학번은 위에서 중복금지 제약조건을 설정아여
-- 중복된 값이 추가되지 못하도록 설정을 했다.

-- 많은 양의 데이터를 추가하다보니
-- 실수로 학생이름, 전화번호 등을 입력하지 않고 추가한 데이터들이 존재한다.
-- 나중에 tbl_student 테이블의 데이터를 사용하여 업부를 수행하려고 했더니
...
-- st_num는 PK로 설정되있기 떄문에
-- st_mum의 값으로 조회를 하면 없거나 1개의 데이터만 보여준다.
