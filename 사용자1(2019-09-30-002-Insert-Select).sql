...
-- 3. tbl_student 에서 이름, 학번, 전화번호, 주소로 칼럼을 리스트에 보이면 좋겠다
...
-- tbl_student 데이터들 중에서 이름(st_name칼럼 값)dl '홍길동'인
-- 데이터들(리스트)를 보여달라
SELECT * FROM tbl_student
...
-- 조회를 한후 특정 칼럼의 값을 응용프로그램에서 사용하려고 할 때
-- 위치(index)로 칼럼 값을 추출했을 때 정확히 원하는 위치 값을 보장하여
-- 오류를 줄일 수 있다.
SELECT * FROM tbl_student;

-- projectin : 칼럼리스트를 나열하는 것
...
-- 어떤 데이터는 '서울시'라고 했다.
-- 이런 경우 서울특별시 라고 조회를 하면 서울시 인 데이터는 보이지 않고
...
-- 학생테이블의 주소칼럼에 저장된 문자열이 '서울'이라는 문자열로 시작되는 데이터들을 보여라
SELECT * FROM tbl_student
...
-- 일정한 길이를 갖고 있을 때
-- 비료연산자( <> )를 사용하여 값을 조회 할 수 있다.
SELECT * FROM tbl_student
WHERE st_tel >= '010-111-0000' AND st_tel <= '010-333-9999';

-- 학생데이터를 조회하고 싶은데
-- 이름은 알지 못하고, 학번이 3번부터 6번 사이에 있었던 것 같다.
-- 이럴 떄 학번 데이터는 문자열 이지만 저장된 구조가 5자리로 일정하므로
-- 비교연산자를 사용하여 데이터를 조회 해볼 수 있다.
SELECT * FROM tbl_student
...
-- 주소가 익산시, 남원시, 해남군 인 모든 데이터를 보고자 할 때
SELECT * FROM tbl_student
