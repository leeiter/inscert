...
-- 현재 Transaction(전체 처리)가 완료되었다는 라는 것을
-- 
COMMIT;
...
-- 이럴 떄는 별도록 PK로 사용할 칼럼ㅇ르 추가해서 '개체무결성'을 보장하는 방법이 있다
DROP TABLE tbl_address; -- 기존테이블을 삭제하고
...
-- 연습이기 떄문에  ROLLBACK 실행하자
ROLLBACK;
SELECT * FROM tbl_address;

-- DELETE를 실행할 떄도 table에 PK가 있으면 반드시 PK 단위로 데이터를 삭제하자
...
SELECT * FROM tbl_address WHERE name = '홍길동'; -- 홍길동데이터는 3개가 출력되어
...
-- 이 명령도 개체무결성을 보장하는데에는 문제가 발생할 소지를 내포한다.
