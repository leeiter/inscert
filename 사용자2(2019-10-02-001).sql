...
-- tbl_address 테이블의 age칼럼에 어떤 값이라도 읿력되어 있으면
-- 그 리스트를 보여달라
SELECT * FROM tbl_address
...
-- 오라클에서는 '' (중간에 빈칸등 아무런 문자열이 없는 형태)는
-- NULL과 같은 의미로 본다.
...
SELECT * FROM tbl_address WHERE address IS NULL; -- address 칼럼이 비어있는 레코들
...
-- 001, 002, 003의 기호값들이 무엇을 의미하는지 알수가 없다.
...
-- 생년월일(),
-- 나이() 와 같이 보고 싶다.
-- projection(칼럼들의 나열)ㅇ,ㄹ 원하는 순서대로 보고자 할 때는
...
-- 데이터를 조외할 떄 특정칼럼을 기준으로 정렬을 수행하여
-- 리스트 보이기
