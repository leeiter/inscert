...
-- tbl_books 테이블을 생성ㅇ르 하고 데이터를 추가하다가 보니
...
-- 삭제 : DROP
-- 변경(수정) : ALTER
-- 상황이 이미 생성된 table의 한 칼럼(b_price)의 자리수를 늘리려고 한다.
...
-- ALTER TABLE 명령ㅇ르 수행할 때 매우 주의할 사항
-- 1. DROP COLUMN
...
-- 나. 타입변경
-- 기존데이터 형식이 변경되면서 데이터가 손실, 소실 될수 있다.
...
-- 이러한 논리적 묹[를 해결하기 위해서 '장르 테이블'을 하나 별도로 생성하고
...
SELECT
    tbl_books.b_isbn, tbl_books.b_title, tbl_books.b_comp, tbl_books.b_writer,
    tbl_books.b_genre,
    -- tbl_genre.g_code,
    tbl_genre.g_name
FROM tbl_books, tbl_genre
...
-- TABLE명에 Aliad를 설정하는 방법
SELECT
    B.b_isbn, B.b_title, B.b_comp, B.b_writer,
    B.b_genre,
    -- tbl_genre.g_code,
    G.g_name
...
VALUES('979-007', '자바의 정석', '도울출판', '남궁선', '004');
