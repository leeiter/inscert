...
-- 서로 연계해서 하나의 리스트러첨 출력하는 SQL 명령형태

-- EQ, 완전, 내부조인
-- 두 테이블에 연계된 칼럼이 모두 존재할 경우
-- 두 테이블간에 완전 참조무결설이 보장되는 경우
-- 이 조인이 표시하는 리스틀 카티션곱이라고 표현한다.
...
-- 하지마, 아직 참조무결성 조건을 설정하지 않았기 때문에 INSERT가 가능하다.

SELECT * FROM tbl_books B, tbl_genre G
WHERE B.b_genre = G.g_code;
-- 그 결과 EQ JOIN으로 확인해 보니 새로 등록한 도서리스트가 누락되어 출력 되었다.
-- 이제 출력 결과는 실뢰성을 잃게 되었다.
...
-- 그렇지 않으면 (null)로 표현하라
SELECT * FROM tbl_books B -- 리스트를 확인하고자하는 table
...
-- 가급적지양을 해야만 데이터의 무결성을 계속해서 유지할 수 있다.
-- 즉, UPDATE, DELETE 등을 수행할때는 1개의 레코드만 변경되도록
-- PK WHERE 조건에 설정해서 명령문을 수행하는 것이 좋다.

-- 이러한 이유로 테이블을 분리하고, 두 테이블을 JOIN 해서 리스트를 출력하는
-- 복잡한 관정을 수행하게 된다.

-- 보통 1개의 테이블에 존재하는 데이터를 다수(2개)의 테이블로 분리하고
-- JOIN을 수행할 수 있도록 구조를 변경하는 작업을 데이터베이스 '제2정규화'