...
-- SELECT를 수행할 때 보고자 하는 칼럼들을 나열 하는것이
-- 효율성 면에서 좋다.
...
-- 참조 무결성이 완전하지 않은 경우 EQ JOIN으로 데이터를 조회하면 안된다.
...
-- GROUP BY
-- 특정칼럼을 기준으로 하여 집계를 할 때 사용하는 명령 절

-- 1. 학과별로 국어점수의 총점을 계산하고 싶다.
-- 각 학과 별로 그룹으로 묶고 국어점수의 총점을 계산 한 후
-- 학과 번호 순으로 보여 달라
-- 부분합 집계
SELECT s_dept, SUM(s_kor)
FROM view_score
GROUP BY s_dept
ORDER BY s_dept;
