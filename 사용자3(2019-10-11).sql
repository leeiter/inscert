-- 여기는 USER3 화면입니다.
-- 현재 USER3 사용자가 사용할 수 있는 TABLE 목록
SELECT
    *
FROM TAB;
...
-- 이런경우 데이터를 조회 했을 때 실제 데이터와 차이가 있는 오류가 날수 있다.

-- tbl_score 테이블과 tbl_student를 JOIN 하고
-- 그 결과에서 학과 코드를 기준으로  다시 tbl_dept와 JOIN을 수행하는 SQL

-- tbl_score 와 tbl_student JOIN
SELECT
    *
FROM tbl_score SC
...
-- 학생수를 기준으로 오름차순 정렬
-- 집계합수(SUM, COUNT, MAX, MIN, AVG)를 사용할때
-- 만약 집계함수로 감싸지 않은 칼럼을 Projection에 표시하려고 하면
-- 그 칼럼들은 반드시 GROUP BY 절에 칼럼들을 나열해주어야 한다.
SELECT 학과코드, 학과이름, COUNT(학과코드)
FROM view_st_dept
GROUP BY 학과코드, 학과이름
ORDER BY COUNT(학과이름);
...
GROUP BY 학과코드, 학과이름
ORDER BY COUNT(학과이름);
-- 1. 전체 데이터에서 학과코드 별로 묶어서, 학과 코드가 무었인지 List
-- 2. LIst내에 포함된 학생수를 계산하여 보기
-- >> 학과코드별 부분 합(개수)을 계산하기

-- 이 SQL에서는 학번은 현재 view에서 절대 중보된 값이 없으므로
...
-- 그 결과에 대하여 조건을 설정한다
-- 만약 어쩐 기존의 칼럼을 기준으로 조건을 설정하려면
-- HAVING 이 아닌 WHERE에서 조건을 설정하여
-- 추출되는 LIST 개수를 줄이고
-- 추출된 LIST만 가지고 GROUP, 집계합수 면산을 수행하는 것이
-- SQL수행 효율면에서 매우 유리하다.
SELECT 학과코드, 학과이름, COUNT(*)
...
(
SELECT
    sc.s_num AS 학번, st.st_name AS 이름, st.st_dept AS 학과코드, sc.s_kor AS 국어, sc.s_eng AS 영어, sc.s_math AS 수학
FROM tbl_score SC
    LEFT JOIN tbl_student ST
        ON sc.s_num = st.st_num
        );
...
SELECT
    sc.s_num, st.st_name, dp.d_name, dp.d_pro, sc.s_kor, sc.s_eng, sc.s_math
FROM tbl_score SC
    LEFT JOIN tbl_student ST
        ON SC.s_num = ST.st_num
    LEFT JOIN tbl_dept DP
...
(
SELECT
    sc.s_num AS 학번, st.st_name AS 학생이름, st.st_dept AS 학과코드, dp.d_name AS 학과이름, dp.d_pro AS 담임교수,
...
-- 전체 학생의 과목별 총점
SELECT
     SUM(국어), SUM(영어), SUM(수학)
FROM view_성적일람표;

-- 학과별로 과목별 총점
SELECT
     학과코드, 학과이름, SUM(국어), SUM(영어), SUM(수학)
FROM view_성적일람표
GROUP BY 학과코드, 학과이름;

SELECT
     학과코드, 학과이름, SUM(국어) AS 국어총점, SUM(영어) AS 영어총점, SUM(수학) AS 수학총점,
...
-- 고정된 길이의 값으로 조건하는 것이 결과를 더 확실하게 찾을 수 있다.
-- 학과 이름으로 조건을 부여하는 것 보다는
-- 학과 코드로 조건을 부여 하는 것이 좋다.
SELECT
     학과코드, 학과이름, SUM(국어) AS 국어총점, SUM(영어) AS 영어총점, SUM(수학) AS 수학총점,
     SUM(총점) AS 전체총점, ROUND(AVG(평균),1) AS 전체평균
FROM view_성적일람표
...
SELECT 학과코드, 학과이름, COUNT(*) 학생수, MAX(총점) AS 최고점, MIN(총점) AS 최저점,
    SUM(총점), ROUND(AVG(평균),1)
FROM view_성적일람표
GROUP BY 학과코드, 학과이름;
