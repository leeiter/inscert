...
UNION ALL SELECT s_num AS 학번, to_char(s_kor, '999') AS 국어,
                                to_char(s_eng, '999') AS 영어, to_char(s_math, '999') AS 수학 FROM tbl_score
UNION ALL SELECT '---' AS 학번, '----' AS 국어, '----' AS 영어, '----' AS 수학 FROM DUAL
UNION ALL SELECT '총점' AS 학번, to_char(SUM(s_kor), '99,999') AS 국어,
                                to_char(SUM(s_eng), '99,999') AS 영어, to_char(SUM(s_math), '99,999') AS 수학 FROM tbl_score
UNION ALL SELECT '===' AS 학번, '====' AS 국어, '====' AS 영어, '====' AS 수학 FROM DUAL;

-- to_char(값, 형식)
-- 숫자형 자료를 문자열형 자료로 변환시켜는 cascading 함수
-- 형식
-- 9 : 숫자자릿수를 나타내는 형식, 실제 출력되는 값의 자릿수만큼 개수를 지정해야 한다.
-- to_char(123, '99999') >> 123
-- 0 : 숫자자릿루를 나타내는 형식, 실제 출력되는 값보다 자릿수가 많으면 나머지는 0으로 채움
...
-- MI : 분
-- SS : 초
-- to_char(날짜값, 'YYYY년 MM월 DD일 HH시 MI분 SS초')
...
SELECT * FROM tbl_student
WHERE st_num IN(SELECT name FROM tbl_temp);

/*
    WHERE 절에 포함된 SELECT 실행 : SELECT name FROM tbl_temp
    '기은성', '남동예', '내세원', '갈한수', '방채호', '배채호' 리스트를 생성하고
    WHERE st_name IN ('기은성', '남동예', '내세원', '갈한수', '방채호', '배채호') 코드를 생성한다.
...
-- 여러 table을 결합하여 나오는 결과값들ㅇ르 모아서 하나의 쿼리로 연결하여
...
-- 단순한 JOIN을 이용해서 보여주기 어려운 복잡한 통계, 집계 등을 사용할 떄
-- 먼저 SUBQ 에서 통계, 집계  등을 계산하고
