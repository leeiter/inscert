...
-- 테이블을 복제하면, 데이터형식, 데이터들 그리고 NOT NULL 등 일부는 그대로 복제가 되는데
...
-- 테이블을 새로 작성하지 않고 제약 조건들만 추가하는 방법
...
-- ALTER TABLE을 사용해서 PK를 추가할 때는 rkrmqwjr dlfmadmf wlwjdgo wnsms rjtdl whgek.
...
-- 4. CHECK 제약조건
-- 데이터를 추가할 떄 어떤 칼럼에 저장되는 데이터를 제한하고자 할때
-- ST_GRADE 칼럼에 값을 1부터 4까지 범위의 숫자만 저장하라
-- 그리고 그 제약 조건이름으로 ST_GRADE_CHECK 라고 등록하라
...
-- 이름이 지정되지 않으면 나중에 조건이 필요 없어서 삭제를 하고자 할 떄
-- 삭제가 어려워 질수도 있다.
