...
5. 가급적이면 1개의 칼럼을 PK로 지정하는 것이 좋고
    정말로 PK로 지정할 칼럼을 선택할 수 없을 때는
    실제 존재하는 데이터가 아닌 새로운 칼럼을 하나 추가 하고
    그 칼럼을 PK로 설정하는 방법도 잇다.
    
    예) '코드' , 'ID'와 같은 칼럼을 추가 해서 사용한다.
...
6. '코드' 칼럼은 최초 데이터를 추가하기 위해 준비단계에서
    ...
    INSERT를 수행할 때마다 자종으로 새로운 숫자 값이 생성되도록 할 수 있다.
    ...
    데이터를 추가할 때 ID  칼럼에 새로운 값이 만들어져서 저장되도록
    사용한다.
    
    오라클의 SEQ는 한번 생성을 하면
    현재 상태를 영구히 보관하고 있다가 언제든지 NEXTVAL을 호출하면
    현재 + (INCTEMENT BY로 지정한 값만큼) 연산을 수행하여
    새로운 값을 만들어 낸다.
*/

/*
오라클에서 RANDOM, SEQ외에 사용할 수 있는

    범 우주적으로 유일한 값
    32Byte의 중복 되지 않는 키 값을 생성

    34F7A3BD816D4A64BABEAF4BEED2BE09

    오라클에서는
    GUID를 저장할 칼럼의 데이터 형식을
    RAW(무한히 크기제한이 없는 바이너리 형태) 값으로 지정하거나
    nVARCHAR(125) 이상으로 지정해서 사용을 한다.
*/
...
해당 칼럼을 index라는 Object로 생성을 해두면
SELECT를 수행할 떄 index를 먼저 조회하고
index로 부터 해당데이터가 저장된 Record의 주소를 얻고
...
SELECT
    *
FROM tbl_books WHERE b_name = '자바';

-- DBMS 업무 중에 b_name(도서명)과 b_writer(저자)를 조건으로 하는
-- SELECT문을 자주 수행하더라 라면
SELECT
    *
FROM tbl_books WHERE b_name = '자바' AND b_writer = '홍길동';
...
INDEXX OBJECT가 문제를 일으키는 상황도 만들어 질 수 있다.
...
-- UNIQE INDEX
-- 마치 table을 생성할때 해당하는 칼럼에 UNIQUE 제약조건을 설정한 것처럼 자동이 된다.
-- 기존에 저장되어 있는 데이터가 UNIQUE 상태가 아니면
-- 인덱스가 생성되지 않는다.
CREATE UNIQUE INDEX IDX_NAME ON tbl_books(b_name);

-- INDEX 가 손상된 것 같다.. DROP 후 CREATe 실행
