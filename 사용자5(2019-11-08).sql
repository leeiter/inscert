...
-- INSERT를 수행할 떄 칼럼을 나열하지 않고
-- (projecttion을 생략할 수 있다.)
...
-- 해당 OBJECT의 특정한 method를 호출하염 일련번호를 생성할 수 있다.
-- SEQUENCE 생성
...
-- 현재 추가된 데이터에서 m_seq가 10인 데이터만확인을 하고 싶다.
SELECT * FROM tbl_memo
WHERE m_seq = 10;
...
-- m_seq가 15인 데이터는 잘못한 것이 발견이 되었다.
...
-- 매입, 매출의 합계를 거래처별로 부분합ㅇ르 구하여 PIVOT방식으로 알아보기
